...
2. 正式科学对象模块采用封闭的 `01–11` 体系；如果论文确实对多个具体科学对象做了实验，可同时进入多个对应模块。
3. Agent 类型、科研流程角色、自主能力、验证方式和交叉属性只作为副标签。
4. 如果论文有具体科学对象实验，即使提出通用科研 Agent 系统，也优先按具体科学对象模块归类，不因平台外观退回 `01`。
5. `01.04` 不再作为正式主类入口，而是独立的 ASD/general 方法存放区，只收纳没有任何具体科学对象实验的文献；科学系统与知识生产研究归入 `11.07`。
