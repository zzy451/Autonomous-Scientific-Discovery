...
2. 优先回到原文或一手来源确认分类证据；本地 Notes 和 legacy 主列表字段只能作为线索，不能作为最终证据。
...
- 第二优先：主列表、已有 Notes、旧 Coverage_Check 报告。它们可帮助定位证据，但不能单独决定多模块分类。
- 如果 Notes 中仍写着旧的单模块或单主类判断，而原文证据支持多模块，应在本轮报告中标记 `note_revision_required = yes`；证据充分且本轮实际触达该 note 时，同步修改 note。
...
不能只参考本地 Notes；Notes 可能仍是旧单模块表述。请尽量核对原文、PDF、arXiv / DOI / publisher 页面、官方项目页或权威摘要，并在输出中写明 `first_hand_sources_checked`。若原文证据支持新增模块或推翻旧 note 表述，请标记 `note_revision_required = yes`。
