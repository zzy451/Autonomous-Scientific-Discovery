# Structured Data Catalog Round 21 Closeout

日期：2026-07-06

## 本轮目标

落实长期方案中二级位 provenance 相关字段，把：

- `secondary_class_source`
- `secondary_class_confidence`
- `secondary_class_review_status`

真正接进结构化导出层、discipline registry、CSV / SQLite 和 checker，而不再只停留在 taxonomy owner 内部。
...
- 若 taxonomy owner 中存在对应 secondary term：
  ...
  - `secondary_class_confidence`
    ...
    - 其余 -> `medium`
  - `secondary_class_review_status`
    - 直接镜像 taxonomy owner `review_status`
- 若 taxonomy owner 中暂时没有对应 secondary term：
...
这样二级位 provenance 已经同时进入 per-paper snapshot 和 discipline current snapshot。
...
- 适配 `build_papers()` 新签名，确保 owner-ledger 维护脚本仍能读取完整 paper snapshot
