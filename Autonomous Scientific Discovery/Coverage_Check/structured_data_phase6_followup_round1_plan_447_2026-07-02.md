...
The queue is already priority-sorted. This round intentionally does **not** attempt to clear all `93` follow-up candidates at once.
...
2. The current top 30 includes:
   - `25` rows still carrying `no_local_pdf` pressure
   - `19` rows still carrying `source_limited=yes`
   - `25` rows still carrying non-full-text evidence pressure
