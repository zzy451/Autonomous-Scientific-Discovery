# ASD Phase 6 follow-up round 7 packet plan

Date: 2026-07-03
Round label: `Phase6FollowupR7`

This plan freezes the next bounded Phase 6 evidence round after the queue refresh that propagated the landed `ASD-0381` delta. No authoritative files are changed by this packetization step.

## 1. Why this round exists

Current repo state already proves:

- queue refresh after `R6` is complete
- the baseline remains `447 / 421 / 26 / canonical 01.04 = 9 / drift 0`
- `ASD-0381` is no longer a high-pressure `source_limited=yes` frontier case

The next natural controller action is therefore to freeze a new bounded evidence round from the refreshed full-text follow-up frontier, rather than silently reusing the old pre-refresh six-paper packet.
...
- retain the same high-pressure chemistry / biology frontier where still appropriate
- drop `ASD-0381` because its landed R6 delta already demoted it out of the high-pressure `source_limited=yes` frontier
- skip `ASD-0547` as the first replacement because it is already an explicit `safety_skip`
- use `ASD-0569` as the cleanest non-safety replacement from the refreshed `85`-score band
...
4. 4 of the 6 remain chemistry automation / robotic-lab anchors whose first-hand access status may still move
5. `ASD-0097` and `ASD-0112` remain important harmonization-pressure checks on whether the refreshed queue and authoritative state stay aligned under another evidence pass
6. `ASD-0569` gives this round a clean post-refresh replacement that does not rely on pretending the old `ASD-0381` frontier position still exists
