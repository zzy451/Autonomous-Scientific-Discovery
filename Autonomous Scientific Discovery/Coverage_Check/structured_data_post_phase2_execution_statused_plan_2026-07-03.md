...
- follow-up queue 已从 stale pre-R5 anchors 刷到 post-R14 authoritative 状态
...
- 已完成 bounded follow-up rounds `R6-R14`
...
- `R14Approx` 已清掉下一组 local archived PDF source-limited pocket
- 当前自然下一步是：**继续在 post-R14 queue 上，以 freshness override 跳过已确认 hard-blocked 的 PDF 路由；对实在找不到 PDF 的论文不再反复硬啃，只要保留可索引、可核真的 HTML / abstract / official-page 证据链即可**
