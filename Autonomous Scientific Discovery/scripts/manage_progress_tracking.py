...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Update one row in the progress owner file "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"Allowed columns: {', '.join(PROGRESS_COLUMNS)}"
        ),
    )
...
def parse_updates(raw_updates: List[str]) -> Dict[str, str]:
    ...
    for raw in raw_updates:
        ...
        if normalized_column not in allowed:
            raise SystemExit(
                f"Unsupported progress column {normalized_column!r}. "
                f"Allowed columns: {', '.join(PROGRESS_COLUMNS)}"
            )
...
def parse_progress_table() -> Tuple[List[str], int, List[Dict[str, str]]]:
    lines = read_text_lossy(PROGRESS_PATH).splitlines()
    ...
    if start_index is None:
        raise SystemExit(f"Header marker not found in {PROGRESS_PATH}")
    ...
    for line in lines[data_start:]:
        if not line.startswith("| ASD-"):
            if rows:
                continue
            continue
        parts = split_markdown_row(line, len(PROGRESS_HEADER))
        rows.append(dict(zip(PROGRESS_HEADER, parts)))
    return lines, start_index, rows


def format_markdown_row(row: Dict[str, str]) -> str:
    cells = [str(row.get(column, "")).strip() for column in PROGRESS_HEADER]
    return "| " + " | ".join(cells) + " |"


def rebuild_progress_file(lines: List[str], header_index: int, rows: List[Dict[str, str]]) -> str:
    data_start = header_index + 1
    if data_start < len(lines) and is_markdown_separator(lines[data_start]):
        data_start += 1

    data_line_indexes = [idx for idx, line in enumerate(lines[data_start:], start=data_start) if line.startswith("| ASD-")]
    ...
    if len(data_line_indexes) != len(formatted_rows):
        raise SystemExit(
            "Progress owner rewrite safety check failed: parsed row count does not match source row count."
        )
...
def main() -> None:
    ...
    lines, header_index, rows = parse_progress_table()
    row_index = next((index for index, row in enumerate(rows) if row["paper_id"] == paper_id), None)
    if row_index is None:
        raise SystemExit(f"paper_id not found in progress owner file: {paper_id}")
    ...
    if not changed_new:
        print("No progress owner changes required.")
        return
    ...
    PROGRESS_PATH.write_text(new_text, encoding="utf-8", newline="\n")
