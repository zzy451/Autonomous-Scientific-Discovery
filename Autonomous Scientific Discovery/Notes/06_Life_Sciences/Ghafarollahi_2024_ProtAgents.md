...
- 四级专题是否为新增：否。
- 归类理由：研究对象是蛋白质序列、结构和力学性质；尽管作者强调 biomaterials，核心对象仍是蛋白质。
- 归类置信度：中-高。
...
- 可能误归类到：`04` 材料科学。
- 最终判定：`06`。
- 判定理由：论文讨论 protein-based biomaterials 和 mechanical properties，但实验任务围绕蛋白序列与结构生成，可作为生命科学/蛋白质科学主类；在材料综述段可作为交叉案例。
- 是否需要二次复核：是，若后续分类更强调 protein materials，可与 `04` 交叉标注。
...
- 可放入哪个章节：生命科学蛋白质设计 Agent；多 Agent 工具编排；计算验证型 Agent。
- 可支撑哪个论点：科学 Agent 可以作为“工具调度层”，把生成模型、物理模拟和分析程序组合成科研工作流。
- 可用于哪个表格或图：Agent 类型与工具链表；验证强度表。
