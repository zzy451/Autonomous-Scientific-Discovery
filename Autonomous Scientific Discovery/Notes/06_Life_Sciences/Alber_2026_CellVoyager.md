...
CellVoyager 是一个面向计算生物学的 AI Agent，用于自主分析生物数据并生成新的科学见解。系统在 Jupyter notebook 环境中工作，基于研究背景、数据和已有分析上下文生成探索蓝图、代码、图表与解释，并根据中间结果迭代后续分析。当前可访问的一手证据稳定锚定其 single-cell / computational-biology 核心，并以 CellBench 与 biological case studies 支撑 `06`；文中带有 human-biomedical 色彩的案例暂只保留为 future source-limited lead，而不是当前已落地 `07`。
...
- 数据集 / 实验对象：稳定可锚定的是 scRNA-seq studies / CellBench / brain aging 等生命科学数据；COVID-19 PBMC 与 human endometrium 相关案例当前只保留为 follow-up lead
