...
- 当前状态：已纳入；当前分类为 source-limited landed note
- 阅读日期：2026-06-22
- 笔记作者：Codex
...
| 科学对象归类 | `06;07`，其中 `06` 稳定，`07` 为 source-limited adjunct module | Nature abstract + preview；official repo README | 论文覆盖 scRNA-seq、CellBench、生物数据分析，并报告 COVID-19、human endometrium、brain aging 等 human-biomedical case studies | 中-高 |
| 方法流程 | 输入背景与数据，生成 exploration blueprints，执行代码，读取输出后继续迭代 | Nature preview；GitHub README | 系统不是单次回答，而是多步分析、执行和解释流程 | 高 |
| 实验验证 | CellBench benchmark + 多个 case studies + expert-style evaluation | Nature abstract / preview data-availability；repo README | 论文报告 76 published scRNA-seq studies 的 benchmark，并有 COVID-19、cell-cell communication、aging 等案例 | 中-高 |
...
- 其他覆盖模块及对应层级路径：`07` 医学与健康科学 -> `07.03/07.02` 邻接的人类生物医学 case-study 覆盖（COVID-19 immune-response、human endometrium、aging-related human biomedical analyses）
- 四级专题是否为新增：否
- 是否进入独立 `01.04` 存放区：否
- 每个模块的对象实验证据：
  - `06`：scRNA-seq、CellBench、single-cell biological data analysis、brain aging biological dataset。
  - `07`：COVID-19 PBMC immune-response case study、human endometrium cell-cell communication case study，以及 human-biomedical results reporting。
- 归类理由：按当前 relaxed rule，只要论文对具体科学对象给出可识别 benchmark / case study / result report，就可记录对应模块。CellVoyager 的稳定核心对象是单细胞与计算生物学，因此 `06` 是稳定主模块；同时其 human-biomedical case studies 已足以支持 `07` 作为 source-limited adjunct module。
- 归类置信度：中。

### 2.2 对象优先判定

- 最终科学研究对象：单细胞转录组数据、细胞状态、immune-response、cell-cell communication、brain aging 等生命科学对象，以及与 COVID-19 等人类生物医学场景相关的 case-study 对象。
- 最终科学问题：Agent 能否自主分析复杂生物数据并提出值得继续验证的生物学 / biomedical insights。
- 为什么不按 Agent 技术、模型方法或发表 venue 归类：LLM、Jupyter 和代码工作流只是实现手段；分类依据是论文实际覆盖的具体生命科学与人类生物医学对象。

### 2.3 容易混淆的边界

- 可能误归类到：独立 `01.04` 通用科研 Agent；仅 `06` 单模块。
- 最终判定：`06;07`
- 判定理由：该论文不是无对象实验的通用方法文献，不能退回 `01.04`；同时也不应继续沿用旧 note 的单模块 `06` 口径，因为当前一手来源已经明确显示其有人类 biomedical case-study 覆盖。
- 多模块覆盖说明：`06` 是稳定核心模块，`07` 不是因为“平台感强”而附会出来，而是来自可识别的 human-biomedical case-study 与结果报告。
- `01.04` 判定说明：不适用，因为论文存在明确具体科学对象 benchmark 与 case study。
- 是否需要二次复核：若后续取得 Nature full text 或可访问 preprint PDF，可进一步提高 `07` 的证据强度；但当前 `06;07` 已可 landed，且需保留 `source_limited=yes`。
...
- 机器人实验：否
- 湿实验：未见当前可访问证据支持
- 临床数据：间接有人类 biomedical case-study 数据
- 真实场景部署：未明确
- 专家评估：是
...
- 关键结果：可访问来源表明其在大量已发表 scRNA-seq studies 上评估，并在多个 biological / biomedical case studies 中报告新见解
- 是否有消融实验：当前可访问证据未充分确认
- 是否有失败案例或负结果：当前可访问证据未充分展开

### 5.3 科学贡献

- 是否发现新知识、新机制、新分子、新材料、新假设或新实验结果：提出新的 biological / biomedical data-analysis insights
...
- 与同一科学领域其他 Agent 文献的关系：可与 SpatialAgent、TransAgent 等生命科学 / biomedical agents 并列比较其对象覆盖范围与验证深度。
- 主要创新点：把 notebook 式计算生物分析流程显式 Agent 化，并在真实 biological / human-biomedical case studies 上展示结果。
...
- 可放入哪个章节：生命科学 Agent；同时可在医学与健康科学交叉覆盖处作为边界案例。
- 可支撑哪个论点：Agent 不仅能处理生物数据，还能在 human-biomedical case studies 中提供可识别结果报告，因此多模块归类不能被旧单模块口径覆盖。
- 可用于哪个表格或图：omics / single-cell agents 对比表；`06/07` 边界案例表。
...
- 需要与哪些论文并列比较：SpatialAgent、TransAgent、其他单细胞 / biomedical discovery agents。

## 9. 总结

### 9.1 一句话概括

面向单细胞生物数据并延伸到 human-biomedical case studies 的 CompBio Agent。
...
2. 生成分析蓝图并执行代码。
3. 读取中间输出并继续迭代。
4. 在 benchmark 与 case study 中给出见解。
...
主展示模块三级类：06.03.04 单细胞生物学 / 计算生物学工作流
主展示模块四级专题：Biology / omics research agents
其他覆盖模块及对应层级路径：07 医学与健康科学 -> human-biomedical case studies
module_assignment_evidence：06 来自 scRNA-seq / CellBench / aging biology；07 来自 COVID-19 PBMC、human endometrium 等 human-biomedical case studies
multi_module_object_coverage_note：06 是稳定核心模块；07 按当前 relaxed rule 作为 source-limited adjunct module 落地
...
验证方式：benchmark; expert_evaluation; human-biomedical case studies
