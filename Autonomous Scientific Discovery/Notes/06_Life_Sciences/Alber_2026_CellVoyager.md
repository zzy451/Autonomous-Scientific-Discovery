...
- PDF / 本地文件路径：当前未在项目 `Reference_PDF/` 中归档本地 PDF；本轮环境下 Nature main-paper PDF 未取得，但官方 supplementary PDF `https://static-content.springer.com/esm/art%3A10.1038%2Fs41592-026-03029-6/MediaObjects/41592_2026_3029_MOESM1_ESM.pdf` 可访问；bioRxiv PDF `https://www.biorxiv.org/content/10.1101/2025.06.03.657517v1.full.pdf` 返回 `403`
...
- R7 follow-up confirms a stronger first-hand source chain: Nature preview / abstract + official Nature supplementary PDF + official GitHub README.
- This strengthens the current landed `06` reading, but does **not** newly land `07`.
- `07` remains only a future source-limited full-text follow-up lead.

## Evidence Log

Round-5 harmonization note: preserve authoritative `06`; keep `source_limited=yes`. R7 follow-up strengthens the source chain to publisher abstract / preview + official Nature supplementary PDF + official repo / demo landing, but the main-paper PDF and bioRxiv PDF still remain unavailable in the current environment.

本轮证据级别：publisher abstract + preview / data-availability + official Nature supplementary PDF + official repo + official demo landing；无本地归档 PDF。
...
| 科学对象归类 | `06` 稳定成立；`07` 仅保留为 future source-limited lead | Nature abstract + preview；official Nature supplementary PDF；official repo README | 论文稳定覆盖 scRNA-seq、CellBench、生物数据分析与单细胞 case studies；human-biomedical case studies 仍值得后续全文复核，但当前不再作为已落地 `07` 模块 | 中高 |
...
| 来源限制 | 无本地 PDF；Nature main-paper PDF 未在当前环境获得；bioRxiv DOI landing 和 PDF 访问受限 | Nature DOI landing；official Nature supplementary PDF；bioRxiv DOI landing；bioRxiv PDF URL | 可以确认一手摘要、官方 supplementary PDF 与配套页面，但还不能声称已读本地全文 PDF | 高 |

## 0. 摘要翻译

CellVoyager 是一个面向计算生物学的 AI Agent，用于自主分析生物数据并生成新的科学见解。系统在 Jupyter notebook 环境中工作，基于研究背景、数据和已有分析上下文生成探索蓝图、代码、图表与解释，并根据中间结果迭代后续分析。当前可访问的一手证据稳定锚定其 single-cell / computational-biology 核心，并以 CellBench 与 biological case studies 支撑 `06`；与 human-biomedical 相关的案例线索当前只保留为 future source-limited follow-up lead，而不是已落地 `07`。
...
- 最终科学研究对象：当前稳定落地的是 single-cell biological data analysis / computational-biology workflow 与相关生命科学对象，而不是已确认落地的医学健康对象。
- 最终科学问题：Agent 能否自主分析复杂生物数据并提出值得继续验证的 biological insights。
- 为什么不按 Agent 技术、模型方法或发表 venue 归类：LLM、Jupyter 和代码工作流只是实现手段；当前 authoritative state 依据的是已稳定锚定的生命科学对象证据，而不是 human-biomedical 语境压力。

### 2.3 容易混淆的边界

- 可能误归类到：独立 `01.04` 通用科研 Agent；`06;07` 当前 landed 多模块。
- 最终判定：`06`
- 判定理由：该论文不是无对象实验的通用方法文献，不能退回 `01.04`；但按当前 harmonization，现有可访问一手来源更稳妥地只支持 `06` 作为 landed module，旧 `07` 仅保留为 future source-limited lead。
- 多模块覆盖说明：旧 `06;07` 讨论已收口为 follow-up lead。若后续获得更强的全文级一手证据，可再重新评估是否足以支持 `07`。
...
- 机器人实验：否
- 湿实验：未见当前可访问证据支持
- 临床数据：当前仅能确认有人类 biomedical 风格案例线索，但本轮只将其作为 future source-limited lead，不据此落地 `07`
- 真实场景部署：未明确
- 专家评估：是

### 5.2 数据、任务与指标

- 数据集 / 实验对象：稳定可锚定的是 scRNA-seq studies / CellBench / brain aging 等生命科学数据；COVID-19 PBMC 与 human endometrium 相关案例当前只保留为 source-limited follow-up mention
- 任务设置：给定研究背景和生物数据后，自主生成并执行分析，比较其是否能产生高质量分析与新见解
- 对比基线：Nature abstract / preview 显示与其他通用模型比较
- 评价指标：benchmark 表现、案例质量、专家认可度
- 关键结果：可访问来源表明其在大量已发表 scRNA-seq studies 上评估，并在多个 biological case studies 中报告新见解；当前 authoritative landed module 只保留 `06`
...
- 可放入哪个章节：生命科学 Agent 主章节；`07` 只保留在 `06/07` 边界 follow-up 队列中
- 可支撑哪个论点：single-cell / computational-biology Agent 可以稳定落在 `06`，而 human-biomedical 风格案例在 source-limited 条件下不应被写成当前已落地 `07`
- 可用于哪个表格或图：omics / single-cell agents 对比表；`06/07` 边界 follow-up 表
- 适合作为代表性案例吗：适合，但应明确标注 `source_limited=yes`。
...
面向单细胞生物数据的 CompBio Agent；当前稳定已落地模块为 `06`，旧 `07` 仅保留为 future source-limited lead。
