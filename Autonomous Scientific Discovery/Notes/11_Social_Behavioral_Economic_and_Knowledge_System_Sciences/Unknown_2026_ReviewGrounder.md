# Unknown 2026 - ReviewGrounder: Improving Review Substantiveness with Rubric-Guided, Tool-Integrated Agents
...
| 科学对象归类 | `11` / `11.07` | 摘要 | 最稳定对象是“rubric-guided、tool-integrated scientific peer-review agents”，而不是单纯的模型方法或发表 venue。 | 高 |
| 方法流程 | 多步 Agent 工作流成立 | 摘要 / 系统描述 | 论文把检索、生成、分析、评估或写作等环节串成可迭代流程。 | 中高 |
| 实验验证 | ReviewBench 与 review substantiveness evaluation | 摘要 / 结果概览 | 当前可得证据显示论文主要通过 ReviewBench 与 review substantiveness evaluation 支撑其主张。 | 中高 |
| 边界判断 | 稳定 11.07；研究对象是同行评审质量提升 | 摘要 / 任务定义 | 当前风险主要集中在边界解释与强度判断，不足以推翻现有主类。 | 中高 |
...
- 二级类：11.07
- 三级类：
- 四级专题：Rubric-guided scientific peer-review agents
- 四级专题是否为新增：否
- 归类理由：按对象优先规则，本文最稳定的研究对象是“rubric-guided、tool-integrated scientific peer-review agents”，因此当前主类保持为 `11` / `11.07`。
- 归类置信度：中高
...
- 可能误归类到：稳定 11.07；研究对象是同行评审质量提升
- 最终判定：保持 `11` / `11.07`
- 判定理由：稳定 11.07；研究对象是同行评审质量提升
- 是否需要二次复核：不强制。
...
围绕“rubric-guided、tool-integrated scientific peer-review agents”组织多步科研工作的 Agent 系统。
