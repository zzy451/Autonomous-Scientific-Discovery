# Unknown 2026 - ARA: Agentic Reproducibility Assessment For Scalable Support Of Scientific Peer-Review
...
论文围绕“scientific reproducibility assessment for scalable peer review support”提出题为《ARA: Agentic Reproducibility Assessment For Scalable Support Of Scientific Peer-Review》的 Agent 系统，核心是把多步科研行动组织成可迭代工作流，并以 ReScience C corpus 与 reproducibility benchmarks 作为主要验证。当前可得证据已经足以支持其 Agent 纳入判断与对象优先归类，但仍应区分“平台泛化叙事”和“最终科学对象”之间的关系。
...
- 是否具备以下至少一项 Agent 能力：
  ...
  - 自主决策：是或部分是
  - 多 Agent 协作：是或部分是
- 在科研流程中承担的明确角色：literature_search_and_reading; tool_use_and_code_execution; evidence_assessment_and_validation; feedback_iteration
...
- 二级类：11.07
- 三级类：
- 四级专题：Agentic reproducibility-assessment systems
- 四级专题是否为新增：否
- 归类理由：按对象优先规则，本文最稳定的研究对象是“scientific reproducibility assessment for scalable peer review support”，因此当前主类保持为 `11` / `11.07`。
- 归类置信度：中高

### 2.2 对象优先判定

- 最终科学研究对象：scientific reproducibility assessment for scalable peer review support
- 最终科学问题：论文试图通过 Agent 系统推进“scientific reproducibility assessment for scalable peer review support”相关研究任务。
...
- 主要角色：literature_search_and_reading; tool_use_and_code_execution; evidence_assessment_and_validation; feedback_iteration
...
- 作者为什么提出该 Agent 系统：希望用 Agent 化流程提升 scientific reproducibility assessment for scalable peer review support 的研究效率与质量。
...
- 人类反馈或专家介入：部分论文存在。
- 实验平台或仿真环境：按 ReScience C corpus 与 reproducibility benchmarks 使用。

## 5. 实验与验证

### 5.1 验证方式

- 当前主要验证：ReScience C corpus 与 reproducibility benchmarks

### 5.2 数据、任务与指标

- 数据集 / 实验对象：围绕“scientific reproducibility assessment for scalable peer review support”的论文设定。
- 任务设置：多步科研工作流中的检索、生成、分析、评估或写作任务。
- 对比基线：以论文原文报告为准。
- 关键结果：当前可得证据表明论文主要通过 ReScience C corpus 与 reproducibility benchmarks 支撑其核心主张。
...
- 可支撑哪个论点：Agent 已经能够围绕“scientific reproducibility assessment for scalable peer review support”形成稳定的多步科研工作流。
...
围绕“scientific reproducibility assessment for scalable peer review support”组织多步科研工作的 Agent 系统。
...
四级专题：Agentic reproducibility-assessment systems
Agent 类型：LLM Agent; Tool-using Agent; Hybrid Agent
科研流程角色：literature_search_and_reading; tool_use_and_code_execution; evidence_assessment_and_validation; feedback_iteration
