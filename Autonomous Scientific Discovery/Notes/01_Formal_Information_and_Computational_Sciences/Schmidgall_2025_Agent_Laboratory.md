...
- PDF / 本地文件路径：`Reference_PDF/01_Formal_Information_and_Computational_Sciences/Schmidgall_2025_Agent_Laboratory.pdf`
...
| 科学对象归类 | `01` 形式、信息与计算科学 | PDF p.1-2 | accepts a human research idea and outputs research report/code repository；具体验证对象是 machine learning research workflow、MLE-Bench tasks 与 code experiments | 高 |
...
- 一级类：`01` 形式、信息与计算科学。
- 二级类：`01.02` 计算机科学 / 机器学习研究自动化。
- 三级类：`01.02.12` / `01.02.13` 人工智能与机器学习研究。
- 四级专题：ML research automation agents。
- 四级专题是否为新增：否。
- 归类理由：虽然系统框架可迁移，但当前原文验证对象是 ML research workflow、MLE-Bench tasks 与代码实验，因此应先归入正式 `01` 模块。
- 归类置信度：高。
...
- 最终判定：`01`。
- 判定理由：在当前 relaxed rule 下，机器学习研究任务和代码实验属于具体 formal/computational research objects，不能继续仅放 `01.04`。
- 是否需要二次复核：否。
