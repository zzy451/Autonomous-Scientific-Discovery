...
- Current authoritative classification: `scientific_object_modules=01`; `general_method_bucket=none`; `primary_module_for_filing=01`; `source_limited=no`; `safety_access_status=local_pdf`
...
**2026-06-21 archive note**: archived project PDF confirmed at `Reference_PDF/01_Formal_Information_and_Computational_Sciences/Gandhi_2025_ResearchCodeAgent.pdf`; current adjudicated filing is `01`, not pure `01.04`, because the validated objects are concrete computational / research-method codification benchmark tasks and this note is no longer source-limited.

| 判断项 | 结论 | 证据位置 | 原文短摘或概括 | 可信度 |
|---|---|---|---|---|
| Agent 纳入 | 是，多 Agent 自动编码研究方法 | 摘要；Fig. 1；方法段 | 使用 planning、research logs、workers、environment，把论文方法转成代码 | 高 |
| 科学对象归类 | `01` 形式、信息与计算科学 / 科研方法编码与计算实现任务 | 摘要；Introduction；Results and Discussion | 研究对象是 research methodologies 的 codification，并以具体计算实现 benchmark 任务验证，不应继续作为纯 `01.04` 泛方法记录 | 高 |
...
- 一级类：`01` 形式、信息与计算科学。
- 二级类：不使用 `01.04`；当前按 `01` 中的形式/计算对象覆盖记录。
- 归档主模块：`01`；本文仍放在当前目录仅为归档便利，不作为分类权威。
- 三级类：科研方法编码 / algorithmic research reproduction。
- 四级专题：Research workflow coding agents。
- 四级专题是否为新增：是，主清单已有新增标记。
- 归类理由：科学对象是科研方法、算法描述和代码实现流程本身，且论文用具体 benchmark 任务验证这些形式/计算对象。
- 归类置信度：高。
...
- 为什么不按 Agent 技术、模型方法或发表 venue 归类：不是因为使用 LLM 归 `01`，而是因为研究对象就是经 benchmark 验证的科研工作流和计算实现任务。

### 2.3 容易混淆的边界

- 可能误归类到：具体方法所属领域，例如计算机视觉或图学习。
- 最终判定：`01`。
- 判定理由：评估任务虽然来自 ML 方法，但论文验证的是具体计算/方法编码任务，因此应落在 `01`，而不是保留为纯 `01.04`。
- 是否需要二次复核：低。
...
- 是否发现新知识、新机制、新分子、新材料、新假设或新实验结果：否；其核心贡献是面向具体计算任务的科研方法实现平台。
- 科学贡献是否经过验证：通过 benchmark 验证。
- 贡献强度判断：中。
- 科学贡献类型：系统平台 / benchmark-oriented method implementation / formal-computational task codification。
- 证据强度：benchmark；足以支持 `01` 对象覆盖而非 `01.04` 泛方法桶。
...
- 可放入哪个章节：形式与计算科学中的科研方法编码 Agent；科研复现与代码执行 Agent。
- 可支撑哪个论点：科学 Agent 不只做实验设计，也能承担经 benchmark 验证的科研方法实现和 reproducibility infrastructure。
