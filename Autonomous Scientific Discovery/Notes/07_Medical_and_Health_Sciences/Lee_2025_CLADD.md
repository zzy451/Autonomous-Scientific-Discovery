...
| 科学对象归类 | `07` 医学与健康科学，药物发现 | Title; Abstract; Sec. 4 | 任务包括 drug-target prediction、property-specific molecular captioning、biological activity prediction | 高 |
...
- 四级专题是否为新增：否。
- 归类理由：系统全部任务服务于 drug discovery，包括分子-靶点、生物活性和分子性质解释。
- 归类置信度：高。
...
- 最终判定：`07`。
- 判定理由：药物靶点、活性、ADMET/性质预测具有药物转化目的。
- 是否需要二次复核：否。
