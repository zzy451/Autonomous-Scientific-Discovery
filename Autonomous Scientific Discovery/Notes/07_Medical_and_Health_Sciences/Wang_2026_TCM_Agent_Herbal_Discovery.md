...
- Local PDF status: no local PDF archived; official publisher HTML/PDF entries are now visibly available on the JPA page, but no first-hand full text or PDF was actually read in the 2026-06-23 pass.

## Phase6FollowupR19 Frozen Adjudication / Source-State Recheck - 2026-07-05

This writeback applies the frozen `Phase6FollowupR19` adjudication for `ASD-0536`.

```text
supported_modules: 07
primary_module_for_filing: 07
source_limited: no
confirmed_source_state: official JPA article page checked; ScienceDirect route checked; visible publisher HTML/PDF entry points confirmed
recheck_result: the concrete object remains TCM network pharmacology, herbal-medicine research, and bioactive-compound discovery, so the note stays a clean `07` landing and the older `source_limited=yes` wording should be cleared
...
- PDF / 本地文件路径：未归档本地 PDF；本轮依据官方 JPA 文章页、ScienceDirect 路由以及可见 HTML full text / PDF 入口按冻结裁决改写为 `source_limited=no`；但这只表示 official-page / visible publisher-route 证据已明确，不表示已有本地 PDF 或已完成全文通读
...
- source_limited：no
- safety/access status：official JPA page and visible publisher routes checked; no local PDF and no completed full-text read claimed

## Evidence Log

| 判断项 | 结论 | 证据位置 | 原文短摘或概括 | 可信度 |
|---|---|---|---|---|
| Frozen adjudication | `07`; `source_limited=no` | Phase6FollowupR19 frozen packet + official JPA / ScienceDirect page recheck | 本轮保持单模块 `07`，不扩展也不回退；官方 JPA 文章页与 ScienceDirect 路由已提供更强的来源状态证据，因此旧的 `source_limited=yes` 说法应清除；但仍不声称已有本地 PDF 或已通读全文 | 高 |
...
| 来源状态 | 全文仍待补强，但已不再按 source-limited 落地 | source refresh summary + Phase6FollowupR19 recheck | 官方 JPA 页面与 ScienceDirect 路由现已可见 HTML full text / PDF 入口；虽然本轮未实际核读对应全文 / PDF，但 landing-page 级一手证据已足以支撑清除旧的 `source_limited=yes` 表述。 | 中高 |

## 0. 摘要翻译

TCM-Agent 面向中医药网络药理学与草药发现任务，构建了一个基于大语言模型的多 Agent 系统。当前已核对的一手来源都把它明确描述为服务于 TCM formulas、herbal medicines 和 bioactive compound discovery 的研究系统，而不是领域无关的通用科研平台。系统强调把检索、推理、数据分析、验证和可视化串起来，用于提升中医药研究与药理发现任务的效率。当前能稳定支撑的是顶层 `07` 医学与健康科学归类；虽然官方 JPA 页面已可见 HTML full text 与 PDF 入口，但本轮并未实际核读对应全文 / PDF，因此本 note 现在采用 `source_limited=no` 的 landing-page 级一手证据口径，同时仍保留“全文待补强”的事实说明。
...
- source_limited：否；本轮已以官方 JPA 页面、ScienceDirect 路由和可见 HTML/PDF publisher entry points 作为稳定来源状态证据；但仍不声称已有本地 PDF 或已完成全文通读
...
- 适合作为代表性案例吗：适合，尤其适合说明官方页与可见 publisher route 已足够支撑对象明确的 `07` 归类，而全文仍待补强
- 推荐引用强度：standard
...
source_limited：no
safety_access_status：official_page_and_visible_publisher_routes_checked; no local PDF and no completed full-text read claimed
