...
| 科学对象归类 | `03;04`，primary filing 保持 `03` | Abstract; Sec. 2.1; Sec. 3 | Chemistry QA / discovery tasks 与化学工具调用稳定支持 `03`；Materials Science split、`pymatgen` 以及 single-/multiple-calling evaluation 则给出独立 `04` 对象证据，不再只作为泛化验证 | 中-高 |
...
- 一级类：`03;04`，primary filing 保持 `03`。
- 二级类：主展示仍为 `03.04` 化学信息学、分子发现与化学 Agent；同时记录 `04` 材料科学覆盖。
- 三级类：化学 / 材料科学工具调用与对象覆盖型 benchmark task。
- 四级专题：Chemistry-and-materials tool-learning agents。
- 四级专题是否为新增：否。
- 归类理由：化学 QA、discovery tasks 与化学工具链稳定支持 `03`；而全文中的 Materials Science split、`pymatgen` 工具以及 single-/multiple-calling evaluation 也给出足够的材料科学对象证据，因此不再把 `04` 写成仅用于泛化展示的次要验证。
- 归类置信度：中-高。

### 2.2 对象优先判定

- 最终科学研究对象：化学分子 / 反应 / 化学工具链，以及 Materials Science split 下的材料科学工具使用对象。
- 最终科学问题：如何让 LLM Agent 可靠调用化学工具完成复杂化学问题求解。
- 为什么不按 Agent 技术、模型方法或发表 venue 归类：决定归类的是化学与材料科学对象 task coverage，而不是 HE-MCTS 或 cs.LG venue 这类方法 / 发表层面标签。

### 2.3 容易混淆的边界

- 可能误归类到：`04` 材料科学；`01.04` 通用科研 Agent。
- 最终判定：`03;04`，primary filing 为 `03`。
- 判定理由：变化点不在于 chemistry-led filing，而在于不再把 Materials Science split 写成仅用于泛化展示的扩展项；当前冻结裁决要求 `04` 作为独立对象模块保留。
- 是否需要二次复核：否；当前 `03;04` 多模块覆盖与 `03` filing 已冻结。
...
- 可放入哪个章节：化学 Agent；工具调用与化学工作流；`03/04` 边界的 multi-module benchmark 型科学 Agent。
- 可支撑哪个论点：化学 Agent 的瓶颈之一是工具选择与参数执行，而非简单增加 LLM 规模；同时 benchmark 对象覆盖也可能让工具型 Agent 同时落在化学与材料科学两端。
- 可用于哪个表格或图：Agent 类型-科研角色矩阵；验证强度表；化学工具池比较表；`03/04` 边界案例表。
- 适合作为代表性案例吗：适合作为化学工具学习 Agent 案例，也适合作为 chemistry-led filing 但已落地 `03;04` 的边界样本，不宜作为闭环实验发现案例。
- 推荐引用强度：普通引用到核心引用之间。
...
主展示模块一级类：03
主展示模块二级类：03.04
主展示模块三级类：化学工具调用与计算化学任务求解
其他覆盖模块及对应层级路径：04 -> Materials Science split / materials tool use / benchmark object coverage
module_assignment_evidence：ChemToolBench chemistry QA / discovery tasks and chemistry tool use support 03；Materials Science split、`pymatgen` 与 single-/multiple-calling evaluation support 04
multi_module_object_coverage_note：keep 03 as filing module because the tool-learning center of gravity remains chemistry-led, but 04 is now a landed materials-science module rather than a secondary generalization check
...
归类置信度：中-高
纳入置信度：高
推荐引用强度：普通引用 / 化学-材料边界工具 Agent 表格核心案例
```
