...
`build_analysis_db.py` now also self-validates key derived analysis surfaces after writing outputs: the module-surface split must remain canonical/workflow/mixed-scope clean, and `discipline_local_code_registry.csv` plus SQLite `discipline_local_code_registry` must remain faithful to the current JSONL snapshot.
