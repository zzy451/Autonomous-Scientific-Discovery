...
- together, `field_ownership_matrix.md`, `discipline_code_assignment_policy.md`, `primary_filing_policy.md`, `check_policy.md`, and the two schema files above are the current frozen governance contracts referenced by the structured-data workflow.
...
- `scripts/run_structured_data_pipeline.py`
  Runs the canonical `export_structured_data.py -> check_data_consistency.py -> build_analysis_db.py` sequence from repo root so routine maintenance follows the project workflow instead of ad hoc command order. The preflight step also reports which of the four owner fact sources are currently changed in the worktree and whether `Data/change_log.jsonl` changed alongside them. Use `--with-execution-audit` to append `audit_execution_definition.py` after the canonical sequence when you want the latest Section 12 audit report refreshed too.
